...
(declare)
(t/ann export-form [t/Any :-> '[(t/Coll t/Any) (t/Coll t/Any)]])
(t/ann var-form? [t/Any :-> Boolean])
...
(defmacro static-case
  ...
  `(case ~val
     ~@(mapcat (t/ann-form
                 (fn [[field thunk]]
                   [(eval field) thunk])
                 [(t/Coll t/Any) :-> '[t/Int (t/U nil (t/Coll t/Any))]])
               (partition 2 cases))
...
(defmacro extend-protocol-fns
  ...
  (let [gen-extend (t/ann-form
                     (fn [type mmap] (list `extend type proto mmap))
                     [(t/U nil t/Sym) t/Any :-> (t/Coll t/Any)])
                   ]
    `(do ~@(for ^{::t/ann (t/Coll t/Any)} [^{::t/ann (t/Coll t/Any)} [type mmap] (partition 2 types+mmaps)]
             (if (coll? type)
               (let [mm (gensym "mm-")]
                 `(let [~mm ~mmap]
